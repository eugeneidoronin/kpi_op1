...
int indices[N_SIDES * N_SIDES] = { 0, 1, 2, 1, 2, 0, 2, 0, 1} ; // indices of indices
/*
i:0   0 1 2
i:1   1 2 0
i:2   2 0 1
...
double bisector(double side1, double side2, double side3, double p);//finds bisector
...
int main() {
    do //cycle to loop the whole program
    {
        ...
        for (int num = 0; num < N_SIDES; num++) {
            ...
            if (!check(lengths[indices[N_SIDES*num]],lengths[indices[N_SIDES*num+1]], \
            lengths[indices[N_SIDES*num+2]])) { //checks the main equality of triangle
            ...
            for (int i = 0; i < N_SIDES; i++) {
                ...
                printf("to %c: height = %.2f cm; bisector = %.2f cm; median = %.2f cm\n", sides[i],
                      height(lengths, i, root), \
                       bisector(side1,side2,side3 , p / 2),
                      medians(side1,side2, side3));
            }
        }
